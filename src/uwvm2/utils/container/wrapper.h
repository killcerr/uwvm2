...
{
	inline constexpr bool operator() (::fast_io::containers::basic_string_view<char_type> str_a, ::fast_io::containers::basic_string_view<char_type> str_b) const noexcept
    {
		return str_a == str_b;
	}
};
...
{
	inline constexpr bool operator() (::fast_io::containers::basic_cstring_view<char_type> str_a, ::fast_io::containers::basic_cstring_view<char_type> str_b) const noexcept
    {
		return str_a == str_b;
	}
};
...
UWVM_MODULE_EXPORT namespace uwvm2::utils::container
{
    ...
    using unordered_flat_set = ::boost::unorder::unordered_flat_set<Key, Hash, Pred, ::uwvm2::utils::container::fast_io_std_allocator<const Key>>;

    template <typename Key, typename Val, typename Hash = ::std::hash<Key>, typename Pred = ::std::equal_to<Key>>
    using unordered_flat_map = ::boost::unorder::unordered_flat_map<Key, Val, Hash, Pred, ::uwvm2::utils::container::fast_io_std_allocator<::std::pair<const Key, Val>>>;
