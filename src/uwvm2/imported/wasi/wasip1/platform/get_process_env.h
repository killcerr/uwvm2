...
# endif
        ...
        // /proc/self/environ is generated by the kernel procfs driver when needed, traversing the memory segment from mm_struct->env_start to env_end within
        // the process.
    ...
    defined(__OpenBSD__)
        ...
        /// @deprecated These systems all support `environ` and retrieve information about the current process, so there's no need to use `sysctl`, which may
        ///             fail.
