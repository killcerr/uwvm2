...
namespace
{
    ...
    bool test_memory_safety()
    {
        ...
        {
            ::fast_io::io::perr("ERROR: Exception caught during memory safety test\n");
            return false;
        }
    }
    ...
    bool test_specific_scenarios()
    {
        try
        {
            // Test 1: Very large graph
            {
                ...
                if(cycles.empty())
                {
                    ::fast_io::io::perr("WARNING: No cycles detected in large graph with known cycle\n");
                }
            }
            ...
            // Test 2: Graph with many self-loops
            {
                ...
                auto cycles = detect_cycles(builder.adj);
                if(cycles.size() != num_nodes)
                {
                    ::fast_io::io::perr("WARNING: Expected ", num_nodes, " self-loop cycles, got ", cycles.size(), "\n");
                }
            }
        ...
        {
            ::fast_io::io::perr("ERROR: Exception caught during specific scenario test\n");
            return false;
        }
    }
    ...
    bool test_memory_corruption_scenarios()
    {
        try
        {
            ...
            {
                GraphBuilder builder;
                std::size_t num_nodes = 100000; // Very large graph
                vector<u8string_view> nodes;
                nodes.reserve(num_nodes);
            ...
            // Test 3: Stress test with rapid allocation/deallocation
            {
                ...
                {
                    GraphBuilder builder;
                    std::size_t num_nodes = 1000 + (iteration % 100);
                    vector<u8string_view> nodes;
                    nodes.reserve(num_nodes);
        ...
        {
            ::fast_io::io::perr("ERROR: Exception caught during memory corruption test\n");
            return false;
        }
    }
